...
export class DBManager{
    ...
    updateToV1(){
        ...
        const stmt = this.m_db.prepare("CREATE TABLE targets("+
    ...
    {
        const stmt = this.m_db.prepare("INSERT INTO targets (user_id,channel_id,gt,lt,dex,asset_id,pool_id,asset_name)"+
    ...
    removeUserTargets(userId:string){
        const stmt = this.m_db.prepare("DELETE FROM targets WHERE user_id=:user_id")
        stmt.run({
            user_id:userId
        })
    }
    removeTargetByAsset(user_id:string,asset_id:number):number[]{
        ...
        const getStmt = this.m_db.prepare("SELECT id FROM targets WHERE user_id=:user_id AND asset_id=:asset_id")
        ...
        const stmt = this.m_db.prepare("DELETE FROM targets WHERE user_id=:user_id AND asset_id=:asset_id")
        stmt.run({
            user_id:user_id,
            asset_id:asset_id
        })
    ...
    countUserTargets(user_id:string):number{
        const stmt = this.m_db.prepare("SELECT COUNT(*) AS count FROM targets WHERE user_id=:user_id")
        let row= stmt.get({
            user_id:user_id
        })
        return row.count;
    ...
    getAllTargets():TrackerTarget[]{
        const stmt = this.m_db.prepare("SELECT id,user_id,channel_id,gt,lt,dex,asset_id,pool_id,asset_name FROM targets");
        const rows = stmt.all();
